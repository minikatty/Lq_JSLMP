...
%                      1<p<2  
%--------------------------------------------------------
function [res,var,flag]=spJSLMP(y,q1,q2,p,sym,x_int)
%count
Max=max(abs(y));
if Max ==0
    res = y;
    ...
    y=y./Max;
    L=length(y);iter_max=5e4;N=L;tol=1e-9;
    n=1;flag=1;H=dctmtx(L);I=eye(L);
% if count>0
%     eta1=5e-2;eta2=3.5e-2;
% else
%     eta1=20e-2;eta2=100e-2;
% end
    eta1=4e-2;eta2=10e-2;
...
if (nargin<6)
   ...
   v_prev=zeros(L,1);                                               %secondary adaptive filter tap weights                                                      %speech deniose  with DCT
...
while n<=iter_max%(n~=1 && norm(w_prev-w_new)>tol && n<=iter_max) || n==1    
    k=mod(n,L)+1; 
    d=y(k);
    X=H(k,:)'; Ik=I(k,:)';    
    w_prev=w_new;
