...
%                      1<p<2  
%--------------------------------------------------------
function [JSLMP]=JSLMSP3(y,q1,q2,p,H,w_true,lambda1,lambda2,sym,initial)
   
L=size(H,1);iter_max=2e4;tol=1e-8;N=size(H,2);
n=1;flag=1;I=eye(L);
...
%H=dctmtx(L);                                                       %speech deniose  with DCT
    tau=1e-9;                                                         % cliping threshold 
%------------------------Adam Parameter+ VSS Setting---------------------------
m1=zeros(N,1);v1=m1;m2=zeros(L,1);v2=m2;epsilon1=1e-8;
alpha=1e-4;beta1=0.9;beta2=1-1e-3;epsilon=1e-8;alpha1=10;
mu=0.00002;mu_max=mu;mu_min=1e-5;mu_pre=mu; label=0;epsilonr=1;
%-----------------------------------------------------------------------------
if (q1==0)
