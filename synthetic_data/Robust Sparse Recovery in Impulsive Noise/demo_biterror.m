...
fprintf('Lasso-ADMM:  elapsed time is %.3f seconds\n',toc(t0));
[mv mi] = min(relerr(1,:));
x_lasso = xx(:,mi,1);  
...
fprintf('Lq-min:\t\t elapsed time is %.3f seconds\n',toc(t0));
...
fprintf('YALL1:\t\t elapsed time is %.3f seconds\n',toc(t0));
[mv mi] = min(relerr(2,:));
x_YALL1 = xx(:,mi,2);   
...
fprintf('Huber-FISTA: elapsed time is %.3f seconds\n',toc(t0));
[mv mi] = min(relerr(3,:));
x_HFISTA = xx(:,mi,3);  
...
fprintf('BP-JP:\t\t elapsed time is %.3f seconds\n',toc(t0));
...
fprintf('BP-SEP: \t elapsed time is %.3f seconds\n',toc(t0));
...
fprintf('Lp-ADM (p=0.5): elapsed time is %.3f seconds\n',toc(t0));
...
fprintf('Lp-ADM (p=0.8): elapsed time is %.3f seconds\n',toc(t0));
